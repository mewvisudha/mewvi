...
if ! grep -q aws_access_key_id ~/.aws/config;
 then
  if ! grep -q aws_access_key_id ~/.aws/credentials;
   then
...
credentails=$(aws iam create-access-key --username "${username}" --query 'AccessKey.[AccessKeyId,SecretAccessKey]' --output text)
